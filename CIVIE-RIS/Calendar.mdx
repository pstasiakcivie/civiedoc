...
<Info>
  The Calendar module is designed to optimize **radiology appointment scheduling** and **resource allocation** for improved efficiency.
</Info>

## Key Features

<Accordion title="Appointment Scheduling">
  Users can **create, modify, reschedule, and cancel** appointments directly from the calendar interface.
</Accordion>

<Accordion title="Resource & Location Management">
  Easily assign appointments to specific **facilities, rooms, and modalities** to ensure proper allocation of resources.
</Accordion>

<Accordion title="Filters & Search">
  Quickly locate appointments by filtering based on **location, resource, status, and captions**.
</Accordion>

<Accordion title="View Options">
  Users can switch between different views to manage schedules effectively:

  * **Day View**: Focus on appointments scheduled for a single day.

  * **Week View**: Get an overview of the entire week's schedule.

  * **List View**: Display appointments in a structured list format.

  * **Status View**: Sort appointments based on their current status.
</Accordion>

<Accordion title="Blocked Slots Management">
  Track unavailable slots due to:
...
<Accordion title="Waitlist Management">
  Patients waiting for available slots can be efficiently managed through the **Waitlist feature**, allowing quick scheduling as slots open up.
</Accordion>
...
<Info>
  Using **filters** helps quickly locate relevant appointments. Ensure the **appointment status** is always updated for better workflow efficiency.
</Info>

<Note>
  Regularly check the **Waitlist** for **rescheduling opportunities** and verify **insurance & eligibility** before confirming appointments.
</Note>
...
<Steps>
  <Step title="Booking an Appointment">
    ...
    4. Add the referring physician or add a new referring physician.

    5. Search and add the required exams.

    6. In the **Documents section**, any available documents for the selected patient will be displayed.

    7. Click **Submit** to finalize the appointment.
  </Step>

  <Step title="Blocking a Slot">
...
<Accordion title="Declining a Blocked Slot">
  ...
  * The **entire series** of a recurring blocked slot (e.g., weekly, monthly).
  ...
  <Steps>
    <Step title="Declining a Single Occurrence">
    ...
    <Step title="Declining an Entire Series">
      1. Right-click on the blocked slot.

      2. Select **Decline** from the menu.

      3. Choose **Series** to remove all occurrences.
...
<Expandable title="View Appointment Details">
...
<Accordion title="Key Information Displayed">
  <Section title="Appointment Status">
    Displayed in the **top-right corner** (e.g., **In Progress**).
  </Section>

  <Section title="Pending Verification / Missing Information">
